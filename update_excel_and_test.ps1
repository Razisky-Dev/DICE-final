...
Write-Host "2. Installing Dependencies (openpyxl)..." -ForegroundColor Cyan
plink -ssh -pw $VPS_PASS -batch ${VPS_USER}@${VPS_IP} "cd /var/www/dice && source venv/bin/activate && pip install -r requirements.txt"

Write-Host "3. Creating Test Order..." -ForegroundColor Cyan
plink -ssh -pw $VPS_PASS -batch ${VPS_USER}@${VPS_IP} "cd /var/www/dice && source venv/bin/activate && python create_test_order.py"
