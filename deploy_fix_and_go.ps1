...
# apt update && apt upgrade -y # unexpected prompts might blocking, maybe skip for speed if user wants "now"
...
# Setup .env if missing (we don't overwrite if exists, but we might want to ensure secrets)
if [ ! -f .env ]; then
    cp .env.example .env
fi
